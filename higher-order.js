...
let evenNumbers = mixedNumbers.filter(x => x % 2 === 0 ? true : false);
...
//Code Here\
let myStrongest = monstersInYourPocket.filter(x => x.CP > 200 ? true : false);
...
*/
let bobsTotal = 0;
let total = purchases.filter(x => x.owner === "Bob")//.map(x => bobsTotal += x.price);
for ( let x = 0; x < total.length; x++ ) {
  bobsTotal += total[x].price;
}
